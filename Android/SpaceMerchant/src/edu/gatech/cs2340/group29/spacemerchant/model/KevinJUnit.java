...
public class KevinJUnit extends TestCase {
	...
	public void testRemove() {
		...
		try {
			...
			assertTrue("Successfully checked for removing from an empty inventory",ret == null);
		}
		catch(Exception e) {
		...
		try {
			ret = inventory.remove(item);
			assertTrue("Successful in removing item from one item inventory", ret == item);
		}
		catch(Exception e) {
		...
		try {
			...
			assertTrue("Successful in removing item from full inventory", ret == item2);
		}
		catch(Exception e) {
